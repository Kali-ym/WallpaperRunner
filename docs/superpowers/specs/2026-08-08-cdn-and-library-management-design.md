...
3. **照片管理**：删单张、设封面、批量删图  
4. **大图浏览增强**：滚轮缩放、双击放大/还原、拖拽平移、工具栏与快捷键（适应窗口 / 1:1）
...
| 收藏 | 仅 ★ 星标 +「仅收藏」筛选 |
| 大图浏览 | 标准缩放：滚轮 + 双击 + 拖拽 + 工具栏 + `+`/`-`/`0` |
| 不做 | 自定义本地标签、已读状态、改磁盘文件夹名、照片拖拽排序、旋转/捏合手势 |
...
### 4.4 大图浏览（Lightbox 增强）

在现有 Lightbox（←/→ 翻页、Esc 关闭、F 全屏）上增加：

| 操作 | 行为 |
| --- | --- |
| 滚轮 | 以指针附近为中心缩放 |
| 双击 | 在「适应窗口」与放大档位之间切换（或放大 ↔ 还原） |
| 拖拽 | 放大后按住拖拽平移 |
| 工具栏 | 放大、缩小、适应窗口、实际大小（1:1） |
| 快捷键 | `+` / `=` 放大，`-` 缩小，`0` 适应窗口（与翻页键不冲突） |

换图（←/→）时重置为「适应窗口」。本版不做旋转、触控板捏合。
...
6. 大图可滚轮/按钮/快捷键缩放，双击切换，放大后可拖拽；换图后恢复适应窗口  
...
- 缩放落在 `src/renderer/components/Lightbox.tsx`（纯前端变换即可，无需主进程）  
- 回归：现有下载队列、代理/curl 通道、Lightbox 翻页/全屏不被破坏  
